...
class TorrentClient:
    ...
    def download_piece(self, peer_ip_and_port: str, piece_index: int, output_file: str):
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as socket:
            ...
            socket.sendall(bit_protocol_req)
            socket.recv(68) # Handshake 
            
            self.wait_for_bitfield(socket)
            self.send_interested_message(socket)
            self.wait_for_unchoke(socket)
...
def main():
    ...
    def download():
        file_name = read_file(root)
        torrent_client = TorrentClient(file_name)
    ...
    selectFileButton = Button(text="Select a torrent file", width=20, height=2, command=download)
